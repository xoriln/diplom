    ...
    const signUpForm = () => (
        ...
        const showSuccess = () => (
            <div className="alert alert-info" style={{display: success ? '' : 'none'}}>
                Новая учетная запись создана. Пожалуйста<Link to="/signin">войдите</Link>
            </div>
        )

    return (
        <Layout title="Страница регистрации" description="" className="container col-md-8 offset-md-2">
            {showSuccess()}
            {showError()}
            {signUpForm()}
        </Layout>
